...
def hyp(code, brand, statement, test_type, metric, vf, vt, test_dim, locked, persona=None, age=None, status="草稿", idea_code=None, idea_label=None, evidence=None, guard=None, trigger=None):
    return {"code":code,"brand":brand,"statement":statement,"test_type":test_type,"metric":metric,
            ...
            "trigger_type":trigger,"owner":"Eling","capacity":"GREEN","tests":[],"verdict":None,
...
HYPS = [
 ...
 hyp("HYP-001","OK188KH","用「提款到账实拍」做主钩,能提升 7 天高价值率","素材","7-Day High-Value Rate","8","12","hook",
     ...
     evidence="历史数据 + 竞品信号",guard="Day-1 Quality Floor",trigger="DATA"),
 ...
 hyp("HYP-002","OK188KH","Format 与 Hook 交叉,找出拉新最优组合","素材","7-Day High-Value Rate","8","11","format,hook",
     ...
     evidence="历史数据",guard="Day-1 Quality Floor",trigger="INTUITION"),
 ...
 hyp("HYP-003","17WINKH","体育钩子 × Format 找量","素材","FDC","12","18","format,hook",
     ...
     evidence="市场研究",guard="Day-1 Quality Floor",trigger="EVENT"),
 ...
 hyp("HYP-004","SBKH","换视觉风格能否提升 CTR","素材","FDC","10","15","visual_style",
     ...
     evidence="纯直觉",guard="Day-1 Quality Floor",trigger="INTUITION"),
 # H5: INZ9 受众测试
 hyp("HYP-005","INZ9","回流唤醒人群对复存激活更敏感","受众","FDC","14","20","",
     ...
     evidence="内部数据",guard="AFDA",trigger="DATA"),
 ...
 hyp("HYP-006","OK188KH","分层红利比首存红利带来更高 AFDA","Promotion","AFDA","6.87","8.5","",
     ...
     evidence="内部数据",guard="AFDA",trigger="STRATEGY"),
]

def cr(gen, hyp_code, hyp_label, label, fmt, hook, vs, offer, gt, aud, age, status, versions=None, runs=None):
    return {"gen_code":gen,"ads_code":(gen.replace("HYP","USC") if status!="待上线" else None),"hyp_code":hyp_code,
            ...
            "spend":None,"fdc":None,"cpa":None,"stc":None,
