...
## PI track 增量:v62–v69(2026-07-08 ~ 08-11)

- **v62–v66 变体标注(Variant)**:对比页在测维度每条素材必填「变体名」——取值相同=区分器(同风格测不同游戏/名人,裁判按 取值+变体 判差异)、取值不同=内容备注;交叉测同取值必须同变体;自动提示(datalist)+保存时大小写归一(已有写法优先→Title Case)。配套:`creatives.variants jsonb`、迁移 17 条素材、删 Visual Style 里 9 个人名/游戏名假词条(Joey/Bryan 为绕过裁判所加,根因是系统缺"同值变体"表达)。
- **v67–v68 Creatives 列表**:🧪 在测维度标记;按假设分组排序(同 hypo 素材连排);删 Spend/FDC/CPA 列、补 Offer/Game Type 列。
- **v65 Monthly Overview** 默认显示当前月(原默认跳到排期最远月份)。
- **v69 新登录页**(V 设计):暗色 hero + 吉祥物鼠标跟随动效;图片 WebP 内嵌压缩(2.28MB→408KB);登录逻辑不变。
- **后端(08-10)**:修复 Daily Reminders split bug(上线以来所有每日提醒被静默丢弃);预算催办改瓶颈催办制(20/23/26/29,催当前卡住一步)+发送计划三连催(25/27/30)+待决策超48h每日兜底;短月末位日期兜底。
