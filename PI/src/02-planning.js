...
function renderCreatives(){
  ...
  document.getElementById('creativeBody').innerHTML=rows.map(c=>`
   <tr class="click" onclick="openCreative(${creatives.indexOf(c)})">
    <td><div class="asset-cell">${thumbBox(c.empty?'empty':c.th,crImgOf(c),c.label,56)}<div style="min-width:0"><span class="code">${c.ads?esc(c.ads):'<span style="color:var(--ink3)">pending</span>'}</span></div></div></td>
    <td>${linkHyp(c)}</td>
...
function renderDict(){
  ...
  document.getElementById('dictBody').innerHTML=(DICT[dictTab]||[]).map((r,i)=>`
   <tr class="${r.on?'':'inactive'}">
    <td class="code"><b style="color:var(--ink)">${esc(r.c)}</b>${hasSc?`<div class="sub" style="display:block;font-size:10.5px" title="广告名短码(命名契约)">短码 ${esc(r.sc||'—')}</div>`:''}</td><td>${esc(r.n)}</td><td>${esc(r.d)}</td><td class="code">${esc(r.u)}</td>
